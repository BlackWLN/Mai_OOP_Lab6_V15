...
TEST(ObserverTest, TextObserverCreatesFile) {
auto bear = NPCFactory::createNPC(KhightType, 0, 0, "Khight");
auto werewolf = NPCFactory::createNPC(DragonType, 1, 1, "Dragon");

auto textObserver = std::make_shared<TextObserver>();
bear->attach(textObserver);

// Уведомляем наблюдателя
bear->notify(werewolf);

// Проверяем, что файл был создан (или дополнен)
...
if (file.good()) {
    ...
    while (std::getline(file, line)) {
        // Продолжаем чтение до последней строки
    }
    EXPECT_NE(line.find("Khight"), std::string::npos);
    EXPECT_NE(line.find("Dragon"), std::string::npos);
    file.close();
}
}
